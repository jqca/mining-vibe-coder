...
const LivePreview: React.FC<Props> = ({ activeUseCase }) => {
  ...
  const isActionMode = activeUseCase.id.includes('safety') || activeUseCase.id.includes('blast') || activeUseCase.id.includes('dust') || activeUseCase.id.includes('seabed');
  ...
  const vizLabels: Record<string, { idle: string; running: string; done: string }> = {
    vein:        { idle: '鉱脈データ待機',         running: '鉱脈探査AI実行中',           done: '鉱脈探査完了' },
    excavation:  { idle: '採掘計画待機',           running: '採掘計画最適化中',           done: '採掘計画完了' },
    grade:       { idle: '品位データ待機',         running: '鉱石品位予測実行中',         done: '品位予測完了' },
    beneficiation:{ idle: '選鉱プロセス待機',      running: '選鉱最適化実行中',           done: '選鉱最適化完了' },
    tailings:    { idle: 'ダムモニタリング待機',   running: '尾鉱ダム安全解析中',         done: '安全解析完了' },
    ventilation: { idle: '換気モデル待機',         running: '坑内換気最適化中',           done: '換気最適化完了' },
    hauling:     { idle: '配車データ待機',         running: '重機配車最適化中',           done: '配車最適化完了' },
    environment: { idle: '環境センサー待機',       running: '環境モニタリング実行中',     done: '環境解析完了' },
    raremetal:   { idle: '回収プロセス待機',       running: 'レアメタル回収最適化中',     done: '回収最適化完了' },
    geology:     { idle: '地質データ待機',         running: '地質モデリング実行中',       done: '地質モデル完了' },
    blast:       { idle: '発破パターン待機',       running: '発破最適化実行中',           done: '発破最適化完了' },
    drainage:    { idle: '排水データ待機',         running: '排水処理最適化中',           done: '排水最適化完了' },
    reserves:    { idle: '埋蔵量データ待機',       running: '埋蔵量推定実行中',           done: '埋蔵量推定完了' },
    iot:         { idle: 'IoTセンサー待機',        running: 'IoT最適化実行中',            done: 'IoT最適化完了' },
    dust:        { idle: '粉じんセンサー待機',     running: '粉じん制御最適化中',         done: '粉じん制御完了' },
    price:       { idle: '価格データ待機',         running: '鉱物価格予測実行中',         done: '価格予測完了' },
    closure:     { idle: '閉山計画待機',           running: '閉山計画最適化中',           done: '閉山計画完了' },
    seabed:      { idle: '海底探査待機',           running: '海底鉱物探査実行中',         done: '海底探査完了' },
    recycle:     { idle: 'リサイクルデータ待機',   running: 'リサイクル最適化中',         done: 'リサイクル最適化完了' },
    safety:      { idle: '安全監視待機',           running: '労働安全AI実行中',           done: '安全評価完了' },
    supply:      { idle: 'サプライチェーン待機',   running: 'SC最適化実行中',             done: 'SC最適化完了' },
  };

  const getVizLabel = () => {
    const type = getVizType(activeUseCase.id);
    const labels = vizLabels[type] || vizLabels.vein;
